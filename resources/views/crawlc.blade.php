...
@php
    $defaultBreadcrumbs = [
        ...
        'Crawler' => backpack_url('plugin/nguonc-crawler'),
    ];
    ...
    <script>
        ...
        $('.btn-load').click(function(e) {
            ...
            const fetchApi = async (link, from, to) => {
                isFetching = true;
                const response = await fetch("{{ backpack_url('plugin/nguonc-crawler/fetch') }}?" +
                    new URLSearchParams({
                        link,
                        from,
                        to
        ...
        const crawl = (el) => {
            ...
            setTimeout(() => {
                processMovie(slug).then(res => {
                    ...
                    $(`.crawling-movie[data-slug="${slug}"] .status`).html(res.payload.message);
        ...
        const processMovie = async (slug) => {
            ...
            const response = await fetch("{{ backpack_url('plugin/nguonc-crawler/crawl') }}", {
                method: 'POST',
                headers: {
        ...
        $("#movies-get-handle").click(async function() {
            const apiDomain = "{{ config('nguonc_crawler.domain', 'https://phim.nguonc.com') }}";
            let params = $("select[name=movies-get-params]").find(":selected").val();

            const response = await fetch("{{ backpack_url('plugin/nguonc-crawler/get-movies') }}", {
                method: 'POST',
                headers: {
                    "Content-Type": "application/json",
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')

...
@php
    $defaultBreadcrumbs = [
        ...
        'Crawler' => backpack_url('plugin/apii-crawler'),
    ];
    ...
    <script>
        ...
        $('.btn-load').click(function(e) {
            ...
            const fetchApi = async (link, from, to) => {
                isFetching = true;
                const response = await fetch("{{ backpack_url('plugin/apii-crawler/fetch') }}?" +
                    new URLSearchParams({
                        link,
                        from,
                        to
        ...
        const processMovie = async (slug) => {
            ...
            const response = await fetch("{{ backpack_url('plugin/apii-crawler/crawl') }}", {
                method: 'POST',
                headers: {
        ...
        $("#movies-get-handle").click(async function() {
            const apiDomain = "{{ config('apii_crawler.domain', 'https://apii.online/apii') }}";
            let params = $("select[name=movies-get-params]").find(":selected").val();

            const response = await fetch("{{ backpack_url('plugin/apii-crawler/get-movies') }}", {
                method: 'POST',
                headers: {
                    "Content-Type": "application/json",
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')

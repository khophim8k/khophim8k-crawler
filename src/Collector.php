...
class Collector
{
    ...
    public function get_kk(): array
    {
        ...
        $data = [
            ...
            'type' => $this->getMovieType($info, $episodes),
            'status' => $info['status'],
            'thumb_url' => $this->getPosterImage($info['slug'], $info['poster_url']), 
            'poster_url' => $this->getThumbImage($info['slug'], $info['thumb_url']),
            'is_copyright' => $info['is_copyright'],
            'trailer_url' => $info['trailer_url'] ?? '',
            'quality' => $info['quality'],
            'language' => $info['lang'],
    ...
    {
        return $this->getImage($slug, $url, Option::get('should_resize_thumb', false), Option::get('resize_thumb_width'), Option::get('resize_thumb_height'));
    }

    public function getPosterImage($slug, $url)
    {
        return $this->getImage($slug, $url, Option::get('should_resize_poster', false), Option::get('resize_poster_width'), Option::get('resize_poster_height'));
    }
    // GetStatusNguonc
    ...
    {
        return $info['category']['1']['list'][0]['name'] == 'Phim bộ' ? 'series' : 'single';
    }
    // End get type nguonc
    protected function getMovieType($info, $episodes)
    {
        return $info['type'] == 'series' ? 'series' : ($info['type'] == 'single' ? 'single' : (count(reset($episodes)['server_data'] ?? []) > 1 ? 'series' : 'single'));
    }
    ...
    protected function getImage($slug, string $url, $shouldResize = false, $width = null, $height = null): string
    {
        ...
        try {
            ...
            $filename = pathinfo($filename, PATHINFO_FILENAME) . '.webp'; // Đổi phần mở rộng sang WebP
            ...
            // Lưu ảnh với định dạng WebP và chất lượng nén
            $img->encode('webp', 80); // Chuyển sang WebP với chất lượng 80%
